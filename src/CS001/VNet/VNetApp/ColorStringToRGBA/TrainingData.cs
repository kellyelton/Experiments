...
{
    public string Id { get; init; } = "UNSET";
    ...
    public static IEnumerable<TrainingData> Load_DataSet_2() {
        using var reader = new StringReader(TRAINING_DATA_SET_2_CSV);
        ...
        while ((line = reader.ReadLine()) is not null) {
            ...
            if (split.Length != 6) throw new InvalidOperationException($"Invalid training data on line {line}: Incorrect value count");

            var ir = int.Parse(split[3]);
            var ig = int.Parse(split[4]);
            var ib = int.Parse(split[5]);
            ...
            // Remove paranths at end
            var pi = name.IndexOf('(');
            if (pi > 0) {
                name = name[..pi];
            }

            //TODO: Randomize Whitespace
            //TODO: Randomize Case
    ...
    public static readonly Lazy<TrainingData[]> Cached_DataSet_2 = new(() => Load_DataSet_2().ToArray());

    // https://github.com/codebrainz/color-names
    // All original code here is released under the GPL v2.0 or later. The data is
    // under whatever license applies to this Wikiepedia entry:
    // http://en.wikipedia.org/wiki/List_of_colors
    // (Appears to be Creative Commons Attribution-ShareAlike 2.0 Unported License)
    private const string TRAINING_DATA_SET_2_CSV = @"
